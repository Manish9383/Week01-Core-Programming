...
public class ShortestAndLongest{
	...
	public static int [] shortestLongest(String [][] split){
	    int [] result={0,0};
		int shortest=Integer.parseInt(split[1][0]);
		int longest=Integer.parseInt(split[1][0]);
		for(int i=0;i<split[0].length;i++){
			if(shortest>Integer.parseInt(split[1][i])){
				shortest=Integer.parseInt(split[1][i]);
			}
			if(longest<Integer.parseInt(split[1][i])){
				longest=Integer.parseInt(split[1][i]);
			}
		}
		result[0]=shortest;
		result[1]=longest;
    ...
    public static void main(String[] args){
        ...
        int [] result=shortestLongest(splitAndCount);
		System.out.println("Length of shortest string in sentence is : "+result[0]+"\nLength of longest string in sentence in : "+result[1]);
